// Добавляем дату в строку с датой инцидента
...
// Добавления файлов и скринов в текстовое поле
var MAX_FILES = 10;
...
function displayImage(file) {
    ...
    reader.onload = function(e) {
        ...
        pathElement.setAttribute("d", "M17 7L7 17M7 7L17 17");
...
function displayFile(file) {
    ...
    reader.onload = function(e) {
        ...
        // File icon or image
        const fileIcon = document.createElement('file-icon');
        ...
        // File name
        const fileName = document.createElement('file-item');
        fileName.className = 'file-name';
...
function showFullImage(src) {
    ...
    // Устанавливаем источник изображения и показываем его
    fullImage.src = src;
    fullImage.style.display = 'block';
    overlay.style.display = 'block';

    // Используем requestAnimationFrame, чтобы стили применились после display: block
...
function closeFullImage() {
    ...
    // Плавное исчезновение изображения и фона
    fullImage.style.opacity = 0;
    overlay.style.opacity = 0;

    // Ожидание завершения анимации перед скрытием
    setTimeout(() => {
...
document.getElementById('exampleFormControlTextarea1').addEventListener('paste', handlePaste);
